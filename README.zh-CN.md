...
- **高级工具工厂** -- 额外提供 MCP、LSP、subagent、task、notebook 等更高阶能力。
...
- `builtinTools` 里的 10 个现成工具

### 仍在演进

- `MCPClient` 与 MCP 动态工具发现
- `createLspTool`、`createSubagentTool`、`createTaskTool`、`notebookEditTool`
- `builtinTools` 之外的高级编排 API 在 `0.x` 期间仍可能调整
...
| 导出 | 产物 | 说明 |
|------|------|------|
| `createLspTool` | `LSP` | 基于调用方提供的 transport 做 LSP 查询 |
| `createSubagentTool` | `Agent` | 派生独立子 agent 处理隔离任务 |
| `createTaskTool` | `TaskCreate` / `TaskUpdate` / `TaskGet` / `TaskList` | 面向多步骤工作的内存任务编排工具集 |
| `notebookEditTool` | `NotebookEdit` | Jupyter notebook 单元增删改 |
